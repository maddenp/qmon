...
(def fmtstr "%-8s %-16s %-8s %-8s %-1s %-8s %-9s %s")

(declare tags tree xml xtime)
...
(defn tree     []     (:content (parse (java.io.StringBufferInputStream. (xml)))))
...
(defn prhead [msg]
  (str (format "\n%s\n\n" msg)
       (format fmtstr "Job ID" "Owner" "Req'd" "Used" "S" "Queue" "Partition" "Job Name\n")
       (format fmtstr (d 8) (d 16) (d 8) (d 8) (d 1) (d 8) (d 9) (d 32))
       "\n"))
